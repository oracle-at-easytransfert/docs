...
L'API KAYBIC AFRICA  utilise les codes HTTP classiques pour signaler l'état général d'une requête. Ces codes ne remplacent pas les erreurs métier, mais permettent d'identifier rapidement si la requête a été reçue et traitée correctement par le serveur.
...
| **500** | Internal Server Error | Il s'agit ici d'une erreur côté serveur KAYBIC AFRICA. Et vous pouvez trouver des explications dans le champ `erreur` dans l'objet `response.data` |
